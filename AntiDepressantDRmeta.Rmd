...
#     ANALYSIS: spline RR
#################################################################
load('antidepOR') # contains the BUGSoutput for nor and bin of linear and spline.
...
## compute gelman Rhat to measure the mixing between the chains
rhatBSOR=c(doseresORsplineBin$BUGSoutput$summary[,'Rhat'])
rhatNSOR=doseresORsplineNor$BUGSoutput$summary[,'Rhat']
...
rvalOR <- cbind(bayesBin=c(beta1bOR,beta2bOR,taubOR,betabOR,taubORL),bayesNor=c(beta1nOR,beta2nOR,taunOR,betanOR,taunORL),Freq=c(beta1fOR,beta2fOR,taufOR,betafOR,taufORL),
      rhatS=c(rhatNSOR,rhatNLOR),rhatS=c(rhatBSOR,rhatBLOR))
...
plot(new.dose1,exp(beta1fOR*new.dose1+beta2fOR*new.dose2),col=1,type='l',ylim = c(0,4)
     ,las=1,ylab='OR',xlab='dose',lwd=3,cex.axis=1.4,cex.lab=1.4) #  freq
lines(exp(beta1nOR*new.dose1+beta2nOR*new.dose2),col=2,lwd=3) # bayes normal
lines(exp(beta1bOR*new.dose1+beta2bOR*new.dose2),col=3,lwd=3) # bayes binomial
