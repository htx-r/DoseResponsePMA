...
makeJAGSDRmeta <- function(studyid, logrr,dose,cases,controls,data,LQ=T){
...
######################################################################
  ...
  if (LQ) {
    ...
    JAGSdata <- list(Y=Ymat,X1=X1mat[,-1],X1ref=X1mat[,1],X2=X2mat[,-1],X2ref=X2mat[,1],X3=X3mat[,-1],X3ref=X3mat[,1],nd=nd,ns=ns,prec=precmat) # X3ref=X3mat[,1],
  }
