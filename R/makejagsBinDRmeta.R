makejagsBinDRmeta <- function(studyid,dose1,dose2,cases,noncases,data,Splines=F,knots){
  ...
  if(Splines){
    ...
    data$X1 <- as.vector(data$dose1)
    data$X2 <- as.vector(data$dose2)
  ...
  ######################################################################
  if (Splines) {
    JAGSdata <- list(r=rmat,n=nmat,X1=X1mat,X2=X2mat,nd=nd,ns=ns)#,prec=precmat) # X3=X3mat[,-1], X3ref=X3mat[,1],

  }else {
    JAGSdata<- list(r=rmat,n=nmat,X=Xmat,nd=nd,ns=ns)#,prec=precmat)
  }
