...
modelBinSplineDRmetaORdrugclusterBiv <- function(){
  for (i in 1:ns) { ## for each study
    ...
    for (j in 2:(nd[i])) { ## for each dose
      ...
      delta[i,j] <-   beta[i,drug[i],1]*(X1[i,j]-X1[i,1]) + beta[i,drug[i],2]*(X2[i,j]-X2[i,1])
    }
  ...
  for(i in 1:ns) {
    beta[i,drug[i],1:2]~dmnorm(betaD[drug[i],1:2],inv.det*(variance*idmat - rho*idmati))
  }
  ...
  for (k in c(1:4,6)) {
    betaD[k,1:2] ~dmnorm(b[1:2], inv.detD*(varianceD*idmat - rhoD*idmati))
  ...
  variance<-tau*tau
  tau~ dnorm(0,1)%_%T(0,)
  inv.det <- 1/(variance^2 + rho^2)

  # prior to covariances
  rho <- cov/variance
  cov ~ dunif(-10,10)
