...
sim.data <- simulateDRsplinedata.fun(beta1.pooled=0.01,beta2.pooled=0.02,tau=0.001)$simulatedDRdata
...
# Freq
rcsplineDRmetaFreq=dosresmeta(formula = logRR~rcs(dose,knots), id = Study_No,type=type,
...
(end-start)*100/60  # it
resultDRsplinemeta1 <- apply(matrix(unlist(res1),n.sim.data,2,byrow = T),2,mean) ##  -0.4206411  0.9744341
resultDRsplinemeta2 <- apply(matrix(unlist(res2),n.sim.data,2,byrow = T),2,mean) ##  -0.4206411  0.9744341
...
for (i in 1:n.sim.data) {
  ...
  sim.data <- simulateDRquadraticdata.fun(beta1.pooled=0.01,beta2.pooled=0.02,tau=0.001)
